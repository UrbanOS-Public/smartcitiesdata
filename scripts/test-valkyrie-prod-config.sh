...
# Test script for validating Valkyrie production configuration locally
# This script builds a production release and checks if it starts successfully

echo "========================================================"
echo "Testing Valkyrie Production Configuration"
echo "========================================================"
...
rm -rf _build/prod/rel/valkyrie
...
env MIX_ENV=prod mix release valkyrie || {
...
if [ -f "_build/prod/rel/valkyrie/releases/1.0.0/runtime.exs" ]; then
    echo "✓ runtime.exs found in release"
else
    echo "✗ WARNING: runtime.exs not found in release!"
    echo "  Expected location: _build/prod/rel/valkyrie/releases/1.0.0/runtime.exs"
fi

# Check sys.config
echo ""
echo "Step 6: Checking sys.config..."
if [ -f "_build/prod/rel/valkyrie/releases/1.0.0/sys.config" ]; then
    echo "✓ sys.config found"
    echo ""
    echo "Checking for brook configuration in sys.config:"
    if grep -q "brook" "_build/prod/rel/valkyrie/releases/1.0.0/sys.config"; then
...
echo "   _build/prod/rel/valkyrie/bin/valkyrie console"
echo ""
echo "2. Test startup as daemon (requires Kafka & Redis running):"
echo "   _build/prod/rel/valkyrie/bin/valkyrie start"
echo "   _build/prod/rel/valkyrie/bin/valkyrie logs"
echo "   _build/prod/rel/valkyrie/bin/valkyrie stop"
echo ""
...
echo "   - 'Valkyrie.Application starting...'"
echo "   - 'Valkyrie Brook configuration:'"
