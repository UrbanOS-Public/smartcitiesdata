defmodule Reaper.XmlSchemaMapperTest do
  ...
  describe "map/2" do
    ...
    test "extracts nested data" do
      schema = [
        %{name: "first_name", type: "string", selector: "./firstName/text()"},
        %{name: "address_of_person", type: "map", subSchema: [
          %{name: "street_name", type: "string", selector: "./address/street/text()"}
        ]}
      ]

      assert XmlSchemaMapper.map(@record, schema) == %{"first_name" => "John", "address_of_person" => %{"street_name" => "Main" }}
    end
    ...
    test "extracts list of maps" do
      schema = [
        %{name: "pets_of_person", type: "list", itemType: "map", selector: "./pets/pet", subSchema: [
          %{name: "pet_name", type: "string", selector: "./@name"},
          %{name: "pet_age", type: "string", selector: "./@age"}
        ]}
      ]

      assert XmlSchemaMapper.map(@record, schema) == %{"pets_of_person" => [%{"pet_age" => "10", "pet_name" => "Simon"}, %{"pet_age" => "12", "pet_name" => "Sally"}]}
