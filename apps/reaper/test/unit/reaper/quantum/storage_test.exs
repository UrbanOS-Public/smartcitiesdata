defmodule Reaper.Quantum.StorageTest do
  ...
  describe "add_job/2" do
    test "persists job into redis" do
      :meck.expect(Redix, :command, fn @conn, ["SET", "reaper:quantum:elixir.reaper.scheduler:job:ingestion_1", _] ->
        {:ok, "OK"}
      end)
      ...
      assert :meck.called(Redix, :command, [
               @conn,
               [
                 "SET",
  ...
  describe "delete_job/2" do
    test "removes the job from redis" do
      :meck.expect(Redix, :command, fn @conn, ["DEL", "reaper:quantum:elixir.reaper.scheduler:job:ingestion_2"] ->
        {:ok, 1}
      end)
  ...
  describe "update_last_execution_date/2" do
    test "save the last execution date to redis" do
      ...
      :meck.expect(Redix, :command, fn @conn,
                                       ["SET", "reaper:quantum:elixir.reaper.scheduler:last_execution_date", _] ->
        {:ok, "OK"}
      end)
  ...
  describe "last_execution_date/1" do
    test "returns last execution date from redis" do
      ...
      :meck.expect(Redix, :command, fn @conn, ["GET", "reaper:quantum:elixir.reaper.scheduler:last_execution_date"] ->
        {:ok, :erlang.term_to_binary(date)}
      end)
      ...
      assert :meck.called(Redix, :command, [
               @conn,
               ["GET", "reaper:quantum:elixir.reaper.scheduler:last_execution_date"]
             ])
    end
