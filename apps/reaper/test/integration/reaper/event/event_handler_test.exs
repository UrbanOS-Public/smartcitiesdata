  ...
  getter(:elsa_brokers, generic: true)
   ...
   describe "Ingestion Update" do
     test "A failing message gets placed on dead letter queue and discarded" do
       ...
       eventually(fn ->
         {:ok, %{"ingestion" => cached_ingestion}} = Brook.ViewState.get(@instance_name, :extractions, id_for_valid_ingestion)
     ...
     test "A failing message gets placed on dead letter queue and discarded" do
       ...
       eventually(fn ->
         ...
         failed_messages = Elsa.Fetch.fetch(elsa_brokers(), "dead-letters")
           ...
           try do
             actual = Jason.decode!(message.value)
             IO.inspect(actual, label: "loggo - inspect after json decode")
             case actual["original_message"] do
               %{"id" => message_ingestion_id} ->
                 message_ingestion_id == id_for_invalid_ingestion
             ...
             error -> IO.inspect(error, label: "loggo - rescue")
           end
         end)
   ...
   describe "Data Extract End" do
     test "A failing message gets placed on dead letter queue and discarded" do
       ...
       id_for_valid_ingestion = UUID.uuid4()
       valid_ingestion = TDG.create_ingestion(%{id: id_for_valid_ingestion})
       allow(Reaper.Collections.Extractions.update_last_fetched_timestamp(id_for_invalid_ingestion), exec: fn _ -> raise "nope" end)
       ...
       eventually(fn ->
         ...
         failed_messages = Elsa.Fetch.fetch(elsa_brokers(), "dead-letters")
           ...
           try do
             ...
             error -> IO.inspect(error, label: "loggo - in rescue of extract end")
           end
         end)
