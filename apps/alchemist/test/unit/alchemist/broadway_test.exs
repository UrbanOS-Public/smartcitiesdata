defmodule Alchemist.BroadwayTest do
  ...
  test "applies alchemist message timing", %{broadway: broadway} do
    Application.put_env(:alchemist, :profiling_enabled, true)
    data = TDG.create_data(dataset_id: @dataset_id, payload: %{"name" => "johnny", "age" => 21})
    kafka_message = %{value: Jason.encode!(data)}
  ...
  test "should yeet message when it fails to parse properly", %{broadway: broadway} do
    ...
    eventually(fn ->
      ...
      assert dlqd_message.app == "Alchemist"
  ...
  test "should emit a data standarization end event when END_OF_DATA message is recieved", %{broadway: broadway} do
    ...
    assert_called(
      Brook.Event.send(@instance_name, data_standardization_end(), :alchemist, %{"dataset_id" => @dataset_id})
    )
  end
end
