defmodule Forklift.SecretRetriever do
  @moduledoc """
  ...
  defp instantiate_vault_conn(token) do
    ...
    |> Vault.auth(%{role: "forklift-role", jwt: token})
  end

  defp set_login_ttl(time, interval), do: NaiveDateTime.utc_now() |> NaiveDateTime.add(time, interval)

  defp get_secrets_endpoint(), do: Application.get_env(:forklift, :secrets_endpoint)
end
