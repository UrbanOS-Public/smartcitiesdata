defmodule Forklift.IngestionProgressTest do
  ...
  describe "IngestionTest" do
    ...
    test "new_messages returns :in_progress if message count *has not* met existing ingestion target", %{
      ...
      Redix.command!(:redix, ["SET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_target", 2])
      result = IngestionProgress.new_messages(1, ingestion_id, dataset.id, extract_time)
    ...
    test "new_messages returns :ingestion_complete if message count *has* met ingestion target", %{
      ...
      Redix.command!(:redix, ["SET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_target", 1])
      result = IngestionProgress.new_messages(1, ingestion_id, dataset.id, extract_time)
    ...
    test "new_messages resets _count and _target if message count *has* met ingestion target", %{
      ...
      Redix.command!(:redix, ["SET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_target", 1])
      IngestionProgress.new_messages(1, ingestion_id, dataset.id, extract_time)
      assert Redix.command!(:redix, ["GET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_target"]) == nil
      assert Redix.command!(:redix, ["GET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_count"]) == nil
    end
    ...
    test "store_target stores target value in redis", %{
      ...
      assert Redix.command!(:redix, ["GET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_target"]) == "7"
    end
    ...
    test "store_target returns :in_progress if count is *less than* new target", %{
      ...
      Redix.command!(:redix, ["SET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_count", 6])
      assert IngestionProgress.store_target(dataset, 7, ingestion_id, extract_time, 1000) == :in_progress
    end
    ...
    test "store_target returns :ingestion_complete if count meets new target", %{
      ...
      Redix.command!(:redix, ["SET", get_extract_id(ingestion_id, dataset.id, extract_time) <> "_count", 3])
