...
#   defp is_in_dlq(message) do
#     # IO.inspect(message, label: "MESSAGEEEE")

#     # |> IO.inspect(label: "Helooooo")
#     {:ok, _, events} = Elsa.fetch([localhost: 9092], "event-stream")

#     # IO.inspect(events, label: "EVENTSSSS")
#     Enum.any?(events, fn event -> String.contains?(event.value, message) end)
#   end
# end
