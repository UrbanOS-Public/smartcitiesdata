defmodule Auth.Auth0.SecretFetcherTest do
  @moduledoc false

  use ExUnit.Case
  use Placebo

  alias Auth.Auth0.SecretFetcher

  @jwks_response   %{"keys" => [
    %{
      "alg" => "RS256",
      "e" => "AQAB",
      "kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ",
      "kty" => "RSA",
      "n" => "v2sPefYhGRLtNg65LivKuqaHtIJo169qEYt7q3EOkfekWKsqqEQBO2BG_RCuZbQ_xW7dzfsthKks7RphP1XxqUiKaEMorMm7WbzjlGmNlaFYjsTrzJ5scATwQvbc7Z8cUYilZmojf2jTt7GUya3Ko6u6KxD_1ixsJSBRNaFy7lizlxoE7sRbPJZw1mVOhHUXLio1AvpOjjWeBVpFfXxfLT2-1_KMxayVwAyIppfOqVflQGeS0Eqz8eWK7vktMvOe1TRSoQEfChWMxg3Q6iDP22m0pgc9QCeVeHBI82q0Cc7NHxPSTWHswVoGqjBFGBrZWoq2qDzBVltwxlV-n2mryQ",
      "use" => "sig",
      "x5c" => [""],
      "x5t" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"
    } ]} |> Jason.encode!()

  describe "valid jwks key store" do
    setup do
      Auth.Auth0.CachedJWKS.clear()

      bypass = Bypass.open()

      Bypass.expect_once(bypass, "GET", "/.well-known/jwks.json", fn conn ->
        Plug.Conn.resp(conn, 200, @jwks_response)
      end)

      [issuer: "localhost:#{bypass.port}"]
    end

    test "gives requested key", %{issuer: issuer} do
      secret = SecretFetcher.fetch_verifying_secret(nil, %{"kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"}, [issuer: issuer])

      assert {:ok, %JOSE.JWK{fields: %{"kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"}}} = secret
    end

    test "returns error message if no key is found in the store", %{issuer: issuer} do
      error_message = SecretFetcher.fetch_verifying_secret(nil, %{"kid" => "does_not_exist"}, [issuer: issuer])

      assert {:error, "no key for kid: does_not_exist"} = error_message
    end
  end

  test "caches key fetching for subsequent calls" do
    Auth.Auth0.CachedJWKS.clear()
    allow(HTTPoison.get(any()), return: {:ok, %{body: "cat"}})

    SecretFetcher.fetch_verifying_secret(nil, %{"kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"}, [issuer: "localhost:doesntmatter"])

    SecretFetcher.fetch_verifying_secret(nil, %{"kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"}, [issuer: "localhost:doesntmatter"])

    assert_called HTTPoison.get(any()), once()
  end

  describe "invalid jwks key store" do
    test "returns error given by issuer" do
      bypass = Bypass.open()

      Bypass.expect_once(bypass, "GET", "/.well-known/jwks.json", fn conn ->
        Plug.Conn.resp(conn, 404, "Not found")
      end)

      error_message = SecretFetcher.fetch_verifying_secret(nil, %{"kid" => "RDVCQjg5MDhENzQ1RTIyMDk1OUI1NEYxODU5MTgwQkVGNDREODlDOQ"}, [issuer: "localhost:#{bypass.port}"])

      assert {:error, _} = error_message
    end
  end
end
