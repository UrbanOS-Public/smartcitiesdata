defmodule RaptorWeb.ListAccessGroupsControllerTest do
  ...
  describe "retrieves access groups by api_key" do
    test "returns an empty list when there are no access groups for the given apiKey", %{conn: conn} do
      api_key = "ap1K3Y"
      user_id = "auth0_user"
      allow(Auth0Management.get_users_by_api_key(api_key), return: {:ok, [%{"user_id" => user_id}]})
    ...
    test "returns a list of access groups when there are access groups for the given apiKey", %{conn: conn} do
      api_key = "ap1K3Y"
      user_id = "auth0_user"
      allow(Auth0Management.get_users_by_api_key(api_key), return: {:ok, [%{"user_id" => user_id}]})
      ...
      actual =
        ...
        |> get("/api/listAccessGroups?api_key=#{api_key}")
        |> json_response(200)
      expected = %{"access_groups" => ["access_group1", "access_group2"]}
