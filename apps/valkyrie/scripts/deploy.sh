...
if [[ $TRAVIS_BRANCH =~ ^[0-9]+\.[0-9]+\.[0-9]+ ]]; then
    export TAGGED_IMAGE="smartcitiesdata/valkyrie:${TRAVIS_BRANCH}"
elif [[ $TRAVIS_BRANCH == "master" ]]; then
    export TAGGED_IMAGE="smartcitiesdata/valkyrie:development"
else
...
docker tag valkyrie:build "${TAGGED_IMAGE}"
