defmodule Valkyrie.ValidatorsTest do
  ...
  describe "schema_satisfied?/2" do
    ...
    test "returns true when nested payload structure matches schema" do
      cooler_schema = [%{name: "name", type: "string"}, %{name: "info", type: "map", subSchema: [%{name: "age", type: "integer"}, %{name: "hometown", type: "string"}, %{name: "hobbies", type: "list", itemType: "string"}]}]
      [msg] = TDG.create_data([dataset_id: "cooler_data", payload: %{
                                  name: "Bobby",
    ...
    test "returns true when a payload containing an array of nested structure matches schema" do
      coolest_schema = [%{name: "name", type: "string"}, %{name: "bio", type: "list", itemType: "map", subSchema: [[%{name: "hometown", type: "string"}, %{name: "pet", type: "string"}],[%{name: "birth_month", type: "string"}, %{name: "books", type: "list", itemType: "string"}]]}]
      [msg] = TDG.create_data([dataset_id: "coolest_data", payload: %{
                                  name: "Hank",
                                  bio: [
    ...
    test "returns false when a payload doesn't match the schema" do
      sad_schema = [%{name: "name", type: "string"}, %{name: "age", type: "integer"}, %{name: "hobbies", type: "list", itemType: "string"}]
