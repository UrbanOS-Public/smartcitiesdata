defmodule DiscoveryApiWeb.DatasetDetailControllerTest do
  ...
  describe "fetch dataset detail" do
    test "maps the data to the correct structure", %{conn: conn} do
      ...
      allow(HTTPoison.get(any(), any()), return: HttpHelper.create_response(body: mock_feed_detail))
    ...
    test "handles HTTPoison errors correctly", %{conn: conn} do
      allow(HTTPoison.get(any(), any()), return: HttpHelper.create_response(error_reason: :econnrefused))
    ...
    test "handles non-200 response codes", %{conn: conn} do
      allow(HTTPoison.get(any(), any()), return: HttpHelper.create_response(status_code: 404))
