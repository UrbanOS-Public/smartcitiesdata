defmodule DiscoveryApiWeb.DatasetPreviewControllerTest do
  ...
  describe "preview dataset data" do
    ...
    test "preview controller returns an empty list for an existing dataset with no data", %{
      conn: conn
    } do
      list_of_columns = ["id", "name"]
    ...
    test "preview controller returns _SOMETHING_ when table does not exist", %{conn: conn} do
      ...
      allow(PrestoService.preview(any()),
        exec: fn _ -> raise Prestige.Error, message: "Test error" end
      )
  ...
  describe "preview restricted dataset" do
    ...
    test "does not preview a restricted dataset if the given user is not a member of the dataset's group",
         %{conn: conn} do
      username = "bigbadbob"
    ...
    test "previews a restricted dataset if the given user has access to it, via cookie", %{
      conn: conn
    } do
      username = "bigbadbob"
      ldap_user = Helper.ldap_user()
    ...
    test "previews a restricted dataset if the given user has access to it, via token", %{
      conn: conn
    } do
      username = "bigbadbob"
      ldap_user = Helper.ldap_user()
