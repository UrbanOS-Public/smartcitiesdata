defmodule DiscoveryApi.Services.QueryCacheTest do
  ...
  describe "fetch_or_execute/2 — cache hit" do
    test "returns cached rows and :cache_hit without calling execute_fn" do
      ...
      expect(RedixMock, :command, fn :redix, ["GET", ^key] -> {:ok, Jason.encode!(@rows)} end)
  ...
  describe "fetch_or_execute/2 — cache miss" do
    test "calls execute_fn, writes SETEX, and returns :cache_miss" do
      key = QueryCache.cache_key(@query)
      expected_json = Jason.encode!(@rows)

      expect(RedixMock, :command, fn :redix, ["GET", ^key] -> {:ok, nil} end)
      expect(RedixMock, :command, fn :redix, ["SETEX", ^key, 360, ^expected_json] -> {:ok, "OK"} end)
      expect(RedixMock, :command, fn :redix, ["INCR", "discovery_api:trino_cache:misses"] -> {:ok, 1} end)
  ...
  describe "fetch_or_execute/2 — Redis failures" do
    ...
    test "does not raise when Redis SETEX fails" do
      stub(RedixMock, :command, fn
        :redix, ["GET", _] -> {:ok, nil}
        :redix, ["SETEX", _, _, _] -> {:error, :econnrefused}
        :redix, ["INCR", _] -> {:ok, 1}
      end)
