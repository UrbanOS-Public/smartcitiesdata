defmodule Pipeline.Writer.S3WriterTest do
  ...
  test "should delete ingestion data from table", %{session: session} do
...
#    }
#
#    ingestion1 = %SmartCity.Ingestion{
#      id: "ingestionId1",
#      targetDatasets: ["dataset1"]
#    }

#    @expected_table_values [
#      %{
#        "tableData" => "tableData1",
#        "_ingestion_id" => "ingestionId1"
#      },
#      %{
#        "Column" => "four",
#        "Comment" => "",
...
#    ]
    ...
    schema = [%{name: "one", type: "string"}, %{name: "two", type: "integer"}, %{name: "_ingestion_id", type: "string"}]
    dataset = TDG.create_dataset(%{technical: %{systemName: "foo__bar", schema: schema}})

    S3Writer.init(table: dataset.technical.systemName, schema: schema, main_partitions: [], json_partitions: [])

    datum1 = TDG.create_data(%{dataset_id: dataset.id, payload: %{"one" => "hello", "two" => 42, "_ingestion_id" => "testId1"}})
    datum2 = TDG.create_data(%{dataset_id: dataset.id, payload: %{"one" => "goodbye", "two" => 9001, "_ingestion_id" => "testId2"}})
...
#               |> execute_query(session) |> IO.inspect(label: "query")
#    end)
#    [dataset: dataset]
#    |> S3Writer.delete()
