defmodule CotaStreamingConsumerWeb.StreamingChannelTest do
  ...
  data_test "sends the user the cache in #{cache} that matches filter given on params when they connect to channel #{channel}" do
    Cachex.put(cache, "12345", %{"vehicleid" => "12345", "type" => "car"})
    Cachex.put(cache, "54321", %{"vehicleid" => "54321", "type" => "bus"})

    {:ok, _, socket} = subscribe_and_join(socket(), CotaStreamingConsumerWeb.StreamingChannel, channel, %{"type" => "bus"})
  ...
  data_test "filter events cause all cached messages in cachee #{cache} to be pushed through filter in channel #{channel}" do
    {:ok, _, socket} = subscribe_and_join(socket(), CotaStreamingConsumerWeb.StreamingChannel, channel)
  ...
  data_test "filter events cause all subsequent messages to be pushed to cache #{cache} through filter in channel #{channel}" do
    {:ok, _, socket} = subscribe_and_join(socket(), CotaStreamingConsumerWeb.StreamingChannel, channel)
  ...
  data_test "filter fields on cache #{cache} with multiple values causes non-matches to be filtered out in channel #{channel}" do
    {:ok, _, socket} = subscribe_and_join(socket(), CotaStreamingConsumerWeb.StreamingChannel, channel)
