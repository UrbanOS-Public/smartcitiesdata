    ...
    test "persists geojson in PrestoDB", %{geo_dataset: ds} do
      ...
      eventually(
        fn ->
          ...
          assert [[actual | _]] = query("select * from #{table}")
          IO.inspect(actual, label: "ACTUAL")
          # Jason.decode!(actual) |> IO.inspect(label: "JASON DECODE RESULT")
          assert "Foo" == "bar"
          # assert result["features"] |> Enum.count() == 88
          # assert result["name"] == "ODOT_County_Boundaries"
