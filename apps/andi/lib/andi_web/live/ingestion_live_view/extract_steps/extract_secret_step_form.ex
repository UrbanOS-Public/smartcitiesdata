defmodule AndiWeb.ExtractSteps.ExtractSecretStepForm do
  ...
  def render(assigns) do
    ~L"""
      <%= f = form_for @changeset, "#", [phx_change: :validate, phx_target: @myself, as: :form_data, phx_submit: :save_secret] %>

        <div class="component-edit-section--<%= @visibility %>">
          <div class="extract-secret-step-form-edit-section form-grid">
            <div class="extract-secret-step-form__destination">
              <%= label(f, :destination, DisplayNames.get(:destination), class: "label label--required", for: "#{@id}__secret_destination") %>
              <%= text_input(f, :destination, [id: "#{@id}__secret_destination", class: "extract-secret-step-form__destination input", required: true]) %>
              <%= ErrorHelpers.error_tag(f, :destination) %>
            </div>

            <div class="extract-secret-step-form__value">
              <%= label(f, :secret_value, DisplayNames.get(:secret_value), class: "label label--required", for: "#{@id}__secret_value") %>
              <%= text_input(f, :secret_value, [id: "#{@id}__secret_value", type: "password", class: "extract-secret-step-form__secret-value input", placeholder: "Secrets are not displayed after being saved", required: true]) %>
