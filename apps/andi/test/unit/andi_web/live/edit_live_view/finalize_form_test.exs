defmodule AndiWeb.EditLiveView.FinalizeFormTest do
  ...
  data_test "converts finalize form data to cadence in form data for #{case}" do
    ...
    where([
      ...
      ["future", finalize_form(%{"cadence_type" => "future"}), %{"technical" => %{"cadence" => "0 0 0 #{future_day()} #{future_month()} * #{future_year()}"}}],
      ...
      ["future with invalid repeating schedule", finalize_form(%{"cadence_type" => "future", "repeating_schedule" => blank_cronlist()}), %{"technical" => %{"cadence" => "0 0 0 #{future_day()} #{future_month()} * #{future_year()}"}}],
      ["future with short time", finalize_form(%{"cadence_type" => "future", "future_schedule" => %{"time" => "00:00"}}), %{"technical" => %{"cadence" => "0 0 0 #{future_day()} #{future_month()} * #{future_year()}"}}],
    ])
  end
end
